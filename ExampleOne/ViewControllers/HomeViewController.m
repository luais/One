...
//    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(nightModeSwitch:) name:@"DKNightVerdionDawnComingNotification" object:nil];
...
{
    if (Is_Night_Mode) {
        self.tabBarController.tabBar.backgroundImage = [self imageWithColor:RGBA(48, 48, 48, 1)];
    }else{
        self.tabBarController.tabBar.backgroundImage = [self imageWithColor:RGBA(241, 241, 241, 1)];
    }
...
- (void)requestHomeContentAtIndex:(NSInteger)index
{
    [HTTPTool requestHomeContentByIndex:index success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (isRefreshing) {
            [self endRefreshing];
            if ([returnHomeEntity.strHpId isEqualToString:((HomeEntity *)readItems[@"0"]).strHpId]) {
                [self showHUDWithText:IsLatestData delay:HUD_DELAY];
            }else{
                //有新数据
                //删掉所有的已读数据，不用考虑第一个已读数据和最新数据之间相差几天
